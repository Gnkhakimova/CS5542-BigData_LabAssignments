...
object IPApp {
  ...
  val IMAGE_CATEGORIES = List("cars", "cats", "dogs", "helicopter", "lighthouse", "panda", "sky", "stairs","strawberry", "zebra")
...
println("test")
  ...
  def generateDecisionTree(sc: SparkContext): Unit = {
    if (Files.exists(Paths.get(IPSettings.RANDOM_FOREST_PATH))) {
      println(s"${IPSettings.RANDOM_FOREST_PATH} exists, skipping Decision Tree model formation..")
      return
    }
